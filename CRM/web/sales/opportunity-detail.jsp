...
<head>
    ...
    <style>
        ...
        .header-row{display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:15px;margin-bottom:25px;}
        ...
        .stage-timeline{display:flex;align-items:center;flex-wrap:wrap;gap:0;margin:20px 0;}
        .stage-step{flex:1;min-width:80px;text-align:center;padding:8px 5px;font-size:11px;font-weight:700;color:#fff;position:relative;cursor:default;}
        .stage-step.active{transform:scale(1.05);z-index:1;border-radius:6px;}
        .stage-Qualification{background:#6c757d;}
        .stage-Need.Analysis{background:#0dcaf0;}
        .stage-Need\.Analysis{background:#0dcaf0;}
        .stage-Product.Proposal{background:#0d6efd;}
        .stage-Quotation{background:#ffc107;color:#000;}
        .stage-Negotiation{background:#fd7e14;}
        .stage-Closed.Won{background:#198754;}
        .stage-Closed.Lost{background:#dc3545;}
        ...
        .modal-box{background:white;border-radius:16px;padding:30px;width:400px;max-width:90vw;}
        .modal-box h4{margin:0 0 20px;font-size:18px;}
        .form-group{margin-bottom:15px;}
        .form-group label{font-size:13px;font-weight:600;color:#555;margin-bottom:5px;display:block;}
        .form-group select,.form-group textarea{width:100%;padding:9px;border:1px solid #ddd;border-radius:8px;font-size:14px;}
        .activity-item{display:flex;gap:12px;padding:10px 0;border-bottom:1px solid #f0f0f0;}
        .activity-icon{width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;color:white;flex-shrink:0;}
        .type-Call{background:#0dcaf0;} .type-Meeting{background:#667eea;} .type-Task{background:#fd7e14;} .type-Note{background:#6c757d;} .type-Email{background:#56ab2f;}
    </style>
</head>
<body>
...
<div class="main-content">
    ...
    <div class="header-row">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <a href="${pageContext.request.contextPath}/sales/opportunity-update?id=${opportunity.id}" class="btn btn-warning"><i class="fas fa-edit"></i> Sửa</a>
            <button class="btn btn-success" onclick="document.getElementById('stageModal').style.display='flex'"><i class="fas fa-exchange-alt"></i> Đổi stage</button>
            <button class="btn btn-danger" onclick="document.getElementById('closeModal').style.display='flex'"><i class="fas fa-flag-checkered"></i> Đóng</button>
            <c:if test="${isManager}">
                <button class="btn btn-info" onclick="document.getElementById('assignModal').style.display='flex'"><i class="fas fa-user-tag"></i> Assign</button>
            </c:if>
            <a href="${pageContext.request.contextPath}/sales/quotation-create?opportunityId=${opportunity.id}" class="btn btn-primary"><i class="fas fa-file-invoice-dollar"></i> Tạo báo giá</a>
        </div>
    </div>
    ...
    <div class="card" style="padding:15px 20px;">
        <div class="stage-timeline">
            <div class="stage-step stage-Qualification    ${opportunity.stage == 'Qualification'    ? 'active' : ''}">Qualification</div>
            <div class="stage-step stage-Need.Analysis    ${opportunity.stage == 'Need Analysis'    ? 'active' : ''}">Need Analysis</div>
            <div class="stage-step stage-Product.Proposal ${opportunity.stage == 'Product Proposal' ? 'active' : ''}">Product Proposal</div>
            <div class="stage-step stage-Quotation        ${opportunity.stage == 'Quotation'        ? 'active' : ''}">Quotation</div>
            <div class="stage-step stage-Negotiation      ${opportunity.stage == 'Negotiation'      ? 'active' : ''}">Negotiation</div>
            <div class="stage-step stage-Closed.Won       ${opportunity.stage == 'Closed Won'       ? 'active' : ''}">Closed Won</div>
            <div class="stage-step stage-Closed.Lost      ${opportunity.stage == 'Closed Lost'      ? 'active' : ''}">Closed Lost</div>
        </div>
    </div>

    <!-- Info Grid -->
    <div class="card">
        <div class="section-title"><i class="fas fa-info-circle" style="color:#667eea"></i> Thông tin chi tiết</div>
        <div class="info-grid">
            <div class="info-item"><label>Khách hàng</label><span>${not empty opportunity.customerName ? opportunity.customerName : 'Chưa liên kết'}</span></div>
            <div class="info-item"><label>Sales phụ trách</label><span>${opportunity.assignedSalesName}</span></div>
            ...
            <div class="info-item"><label>Giá trị dự kiến</label><span class="val-currency"><fmt:formatNumber value="${opportunity.expectedValue}" type="number" groupingUsed="true"/> đ</span></div>
            <div class="info-item"><label>Xác suất đóng</label><span>${opportunity.closeProbability}%</span></div>
            <div class="info-item"><label>Ngày dự kiến đóng</label><span><fmt:formatDate value="${opportunity.expectedCloseDate}" pattern="dd/MM/yyyy"/></span></div>
            <div class="info-item"><label>Nguồn</label><span>${opportunity.source}</span></div>
            <c:if test="${not empty opportunity.campaignName}">
                <div class="info-item"><label>Chiến dịch</label><span>${opportunity.campaignName}</span></div>
            </c:if>
            <c:if test="${not empty opportunity.lostReason}">
                <div class="info-item"><label>Lý do thất bại</label><span style="color:#dc3545">${opportunity.lostReason}</span></div>
            </c:if>
        </div>
    ...
    <div class="card">
        ...
        <c:choose>
            <c:when test="${empty quotations}">
                <p style="color:#aaa;text-align:center;">Chưa có báo giá nào. <a href="${pageContext.request.contextPath}/sales/quotation-create?opportunityId=${opportunity.id}">Tạo báo giá đầu tiên</a></p>
            </c:when>
            <c:otherwise>
                <table>
                    <thead><tr><th>Mã báo giá</th><th>Phiên bản</th><th>Status</th><th>Tổng tiền</th><th>Hết hạn</th><th></th></tr></thead>
                    <tbody>
                        <c:forEach var="q" items="${quotations}">
                            <tr>
                                ...
                                <td><fmt:formatNumber value="${q.totalAmount}" type="number" groupingUsed="true"/> đ</td>
                                <td><fmt:formatDate value="${q.validUntil}" pattern="dd/MM/yyyy"/></td>
                                <td><a href="${pageContext.request.contextPath}/sales/quotation-detail?id=${q.id}" class="btn btn-info"><i class="fas fa-eye"></i></a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
    ...
    <div class="card">
        <div class="section-title" style="justify-content:space-between;">
            ...
            <a href="${pageContext.request.contextPath}/sales/activity-create?opportunityId=${opportunity.id}" class="btn btn-primary" style="font-size:12px;padding:7px 12px"><i class="fas fa-plus"></i> Thêm</a>
        </div>
        <c:choose>
            <c:when test="${empty activities}"><p style="color:#aaa;text-align:center;">Chưa có hoạt động nào.</p></c:when>
            <c:otherwise>
                <c:forEach var="act" items="${activities}">
                    <div class="activity-item">
                        ...
                        <div>
                            <strong>${act.title}</strong> <span style="background:#f0f0f0;padding:2px 7px;border-radius:10px;font-size:11px;">${act.type}</span>
        ...
        <form method="post" action="${pageContext.request.contextPath}/sales/opportunity-stage">
            ...
            <div class="form-group">
                ...
                <select name="newStage">
                    ...
                    <option value="Need Analysis"    ${opportunity.stage == 'Need Analysis'    ? 'selected' : ''}>Need Analysis</option>
                    <option value="Product Proposal" ${opportunity.stage == 'Product Proposal' ? 'selected' : ''}>Product Proposal</option>
                    <option value="Quotation"        ${opportunity.stage == 'Quotation'        ? 'selected' : ''}>Quotation</option>
            ...
            <div style="display:flex;gap:10px;justify-content:flex-end;">
                <button type="button" class="btn btn-warning" onclick="document.getElementById('stageModal').style.display='none'">Hủy</button>
...
<div id="closeModal" class="modal-overlay" onclick="this.style.display='none'">
    <div class="modal-box" onclick="event.stopPropagation()">
        ...
        <form method="post" action="${pageContext.request.contextPath}/sales/opportunity-close">
            ...
            <div class="form-group">
                <label>Kết quả</label>
                <select name="status" onchange="document.getElementById('lostReasonGroup').style.display=this.value=='Lost'?'block':'none'">
            ...
            <div style="display:flex;gap:10px;justify-content:flex-end;">
                <button type="button" class="btn btn-warning" onclick="document.getElementById('closeModal').style.display='none'">Hủy</button>
