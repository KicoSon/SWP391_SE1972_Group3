                ...
                <table class="activity-table">
                    ...
                    <tbody>
                        <c:forEach items="${activities}" var="act">
                            <tr>
                                <td><fmt:formatDate value="${act.createdAt}" pattern="dd/MM/yyyy HH:mm" /></td>

                                <td>
                                    <span class="type-badge type-${fn:toLowerCase(act.type)}">
                                        <c:choose>
                                            <c:when test="${act.type == 'Call'}"><i class="fas fa-phone-alt"></i></c:when>
                                            <c:when test="${act.type == 'Email'}"><i class="fas fa-envelope"></i></c:when>
                                            <c:when test="${act.type == 'Meeting'}"><i class="fas fa-users"></i></c:when>
                                            <c:when test="${act.type == 'Task'}"><i class="fas fa-tasks"></i></c:when>
                                            <c:when test="${act.type == 'Note'}"><i class="fas fa-sticky-note"></i></c:when>
                                            <c:otherwise><i class="fas fa-briefcase"></i></c:otherwise>
                                        </c:choose>
                                        ${act.type}
                                    </span>
                                </td>
                                ...
                                <td class="text-truncate" style="max-width: 150px;" title="${act.description}">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${not empty act.customerName}">
                                            <span style="color: #2e5bff; font-weight: 500;"><i class="fas fa-building"></i> ${act.customerName}</span>
                                        </c:when>
                                        <c:when test="${not empty act.leadName}">
                                            <span style="color: #00b8d9; font-weight: 500;"><i class="fas fa-bullseye"></i> ${act.leadName}</span>
                                        </c:when>
                                        <c:otherwise>
                                ...
                                <td>
                                    <c:if test="${not empty act.dueDate}">
                                        <span class="due-date">
                                            <i class="far fa-calendar-alt"></i> <fmt:formatDate value="${act.dueDate}" pattern="dd/MM/yyyy HH:mm" />
                                        </span>
                                    </c:if>
                                </td>

                                <td>
                                    <span class="status-badge status-${fn:toLowerCase(fn:replace(act.status, ' ', '-'))}">
                                        ${act.status}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <span class="badge badge-light">
                                        <i class="fas fa-user-circle"></i> ${not empty act.assigneeName ? act.assigneeName : 'Chưa phân công'}
                                    </span>
                                </td>

                                <td>
                                    <span style="color: #666; font-size: 0.9em;">
                                        ${not empty act.creatorName ? act.creatorName : 'Hệ thống'}
                                    </span>
                                </td>

                                <td class="action-cell">
                                    <a href="${pageContext.request.contextPath}/activities/detail?id=${act.id}" class="action-btn view-btn" title="Xem chi tiết"><i class="fas fa-eye"></i></a>
                            </tr>
                        </c:forEach>
